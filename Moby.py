...
DEFAULT_COSTS = {
    ...
    "markup_totale": 2.5, # Default Markup
        ...
        if os.path.exists("tempicosti_default.json"):
            try:
                with open("tempicosti_default.json", "r") as f:
                    # Carico e faccio merge per evitare chiavi mancanti
                    loaded = json.load(f)
...
def calculate_quote_logic(stats, user_inputs):
    ...
    if not user_inputs['stock_iron']: 
        days_iron += cfg.get('gg_ordine_ferro', 1) + cfg.get('gg_arrivo_lastra', 5)
    days_iron += cfg.get('gg_verniciatura_ferro', 5)
    
    # Legno (convertiamo min in ore in giorni)
    ...
    days_wood = days_wood_work + cfg.get('gg_verniciatura_legno', 3)
    ...
    log_cost = 0.0
    log_days = 0
    hrs_packing = cfg.get('ore_imballo_base', 1.0)
...
# --- 6. DXF & STL ENGINE (TUE ORIGINALI) ---
def create_dxf_doc():
    doc = ezdxf.new()
...
with tab3:
    ...
    with st.expander("⏱️ Tempi Approvvigionamento (Giorni)", expanded=False):
        ...
        st.session_state.costs_config['gg_verniciatura_ferro'] = c3.number_input("Verniciatura Ferro", value=st.session_state.costs_config.get('gg_verniciatura_ferro', 5))

    with st.expander("🔨 Tempi Lavorazione (Minuti)", expanded=False):
        c1, c2 = st.columns(2)
        st.session_state.costs_config['min_taglio_legno_pezzo'] = c1.number_input("Taglio Legno (pz)", value=st.session_state.costs_config.get('min_taglio_legno_pezzo', 0.0))
        st.session_state.costs_config['min_colore_legno_metro'] = c2.number_input("Colore Legno (m)", value=st.session_state.costs_config.get('min_colore_legno_metro', 0.0))
        c3, c4 = st.columns(2)
        st.session_state.costs_config['min_preassemblaggio_modulo'] = c3.number_input("Pre-ass. Modulo", value=st.session_state.costs_config.get('min_preassemblaggio_modulo', 0.0))
        st.session_state.costs_config['min_preassemblaggio_mensola'] = c4.number_input("Pre-ass. Mensola", value=st.session_state.costs_config.get('min_preassemblaggio_mensola', 0.0))
    
    # 3. CALCOLO E OUTPUT
    # Ricalcolo stats veloci basate sul Tab 2 (che potrebbe non essere stato renderizzato se siamo subito nel Tab 3)
    # Copio logica di calcolo stats
    ...
    # 4. DOWNLOAD PREVENTIVO
    proj_data_comm = {"project_name": st.session_state['project_name'], "num_colonne":num_colonne, "finish_wood": st.session_state['finish_wood'], "finish_iron": st.session_state['finish_iron']}
    client_data_comm = {"name": st.session_state['client_name'], "address": st.session_state['client_address']}
    
    if st.button("📄 GENERA PREVENTIVO CLIENTE (PDF)", type="primary"):
        pdf_comm = generate_commercial_pdf(proj_data_comm, totals, client_data_comm)
        st.download_button("SCARICA PREVENTIVO", pdf_comm, "Preventivo.pdf", "application/pdf")
        
    st.markdown("---")
    st.download_button("💾 Salva Configurazione Prezzi (Default)", json.dumps(st.session_state.costs_config), "tempicosti_default.json", "application/json")
