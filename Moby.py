...
VERSION = "v24.2 Restored & Powered"
...
# --- 3. GESTIONE TEMPI & COSTI (DEFAULT ARRICCHITO) ---
DEFAULT_COSTS = {
    "costo_ferro_kg": 2.0,
    "costo_legno_mq": 30.0,
    "costo_ora_operaio": 35.0,
    "markup_totale": 2.5, # NUOVO: Ricarico vendita
    
    "gg_ordine_ferro": 1,
    "gg_arrivo_lastra": 5,
    "gg_verniciatura_ferro": 5,
    "gg_verniciatura_legno": 3, # NUOVO
    "gg_attesa_corriere": 2, # NUOVO
    
    "min_taglio_legno_pezzo": 5.0,
    "min_colore_legno_metro": 15.0,
    "min_finitura_legno_metro": 10.0,
    "min_preassemblaggio_modulo": 30.0,
    "min_preassemblaggio_mensola": 5.0,
    
    "ore_pulizia": 2.0,
    "ore_imballo_base": 1.0, # NUOVO
    "ore_imballo_extra": 2.0, # NUOVO (per spedizione)
    "costo_imballo_materiale": 20.0, # NUOVO
    
    "ore_prep_spedizione": 2.0,
    "km_viaggio": 0,
    "ore_montaggio_in_loco": 4.0,
    "num_montatori": 2
}
        ...
        if os.path.exists("tempicosti_default.json"):
            try:
                with open("tempicosti_default.json", "r") as f:
                    loaded = json.load(f)
                    # Merge per evitare chiavi mancanti se il file è vecchio
...
# --- 4. PDF ENGINE (AGGIORNATO CON COMMERCIALE) ---
...
def generate_pdf_report(project_name, parts_list, wood_data, iron_data, stats, cols_data, colors):
    # VERSIONE TECNICA (TUA ORIGINALE)
    ...
    pdf.set_fill_color(240, 240, 240)
    pdf.set_font("Arial", 'B', 11)
    pdf.cell(0, 8, "DISTINTA MATERIALI", 0, 1, 'L', fill=True)
    pdf.ln(2)
    ...
    pdf.ln(10)
    pdf.set_font("Arial", 'B', 11)
    pdf.cell(0, 8, "LISTA PEZZI LEGNO", 0, 1, 'L', fill=True)
    pdf.set_font("Arial", size=9)
    if not wood_data.empty:
        for index, row in wood_data.iterrows():
            pdf.cell(40, 8, f"{row['Larghezza']:.0f} x {row['Profondità']:.0f} cm", 1)
            pdf.cell(40, 8, f"Qta: {row['Pezzi']}", 1, 1)
    
    # PAG 3 - ESECUTIVI TAGLIO
    pdf.add_page()
    pdf.set_fill_color(240, 240, 240)
    pdf.set_font("Arial", 'B', 11)
    pdf.cell(0, 8, "ESECUTIVI TAGLIO (FERRO)", 0, 1, 'L', fill=True)
    pdf.ln(10)
    scale_cut = 0.5 
...
def calculate_quote(stats, user_inputs):
    ...
    days_iron += cfg.get('gg_taglio_ferro', 1) + cfg['gg_verniciatura_ferro']
    
    # Percorso Legno
    # Convertiamo tutto in ore e poi in giorni lavorativi
    mins_legno = (stats['viti']/6 * cfg['min_taglio_legno_pezzo']) + (mq_legno * cfg['min_colore_legno_metro'])
    ...
    mins_pre = (user_inputs['num_cols'] * cfg['min_preassemblaggio_modulo']) + (stats['viti']/6 * cfg['min_preassemblaggio_mensola'])
    hrs_prod_tot = hrs_legno + (mins_pre / 60.0)
    cost_labor_prod = hrs_prod_tot * cfg['costo_ora_operaio']
    ...
    if user_inputs['logistics_type'] == "corriere":
        hrs_packing += cfg['ore_imballo_extra']
        ...
        log_cost += tot_man_hrs * cfg['costo_ora_operaio']
        log_days += 1
    ...
    price_ex_vat = base_markup * cfg['markup_totale']
...
# --- 6. LOADING & DXF UTILS (TUE ORIGINALI) ---
def create_dxf_doc():
    doc = ezdxf.new()
...
with tab2:
    ...
    fig_all.update_layout(xaxis=dict(title="Lunghezza (cm)", showgrid=True), yaxis=dict(showgrid=False, zeroline=False, showticklabels=False), height=600, margin=dict(l=10, r=10, t=10, b=10), showlegend=False)
...
with tab3:
    ...
    st.info("I dati inseriti qui aggiornano il calcolo finale. Salva la configurazione per il futuro.")
    
    # AGGIUNTO: Checkbox Logica Stock e Logistica
    col_setup1, col_setup2 = st.columns(2)
    with col_setup1:
        st.subheader("1. Tempistiche")
    ...
    with col_setup2:
        ...
        log_type = st.radio("Metodo", ["Corriere", "Nostro Montaggio"])
        ...
        if log_type == "Corriere":
            ...
            gg_viaggio_corr = st.number_input("GG Viaggio", 1, 10, 2)
        else:
            ore_viaggio = st.number_input("Ore Viaggio (A/R)", 0.0, 20.0, 2.0)
            ore_montaggio = st.number_input("Ore Montaggio in loco", 0.0, 40.0, 4.0)
            num_op = st.number_input("Num. Operai", 1, 4, 2)

    st.write("---")
    
    # Expanders originali aggiornati con Markup
    with st.expander("🛠️ Costi Base e Ricarico", expanded=True):
        ...
        st.session_state.costs_config['markup_totale'] = c4.number_input("Markup (X)", value=st.session_state.costs_config.get('markup_totale', 2.5), step=0.1)
    ...
    with st.expander("⏱️ Tempi Approvvigionamento (Giorni)", expanded=False):
        ...
        st.session_state.costs_config['gg_verniciatura_ferro'] = c3.number_input("Verniciatura", value=st.session_state.costs_config.get('gg_verniciatura_ferro', 5))

    with st.expander("🔨 Tempi Lavorazione (Minuti)", expanded=False):
        c1, c2 = st.columns(2)
        st.session_state.costs_config['min_taglio_legno_pezzo'] = c1.number_input("Taglio (pz)", value=st.session_state.costs_config.get('min_taglio_legno_pezzo', 5.0))
        st.session_state.costs_config['min_colore_legno_metro'] = c2.number_input("Colore (m)", value=st.session_state.costs_config.get('min_colore_legno_metro', 15.0))
        c3, c4 = st.columns(2)
        st.session_state.costs_config['min_preassemblaggio_modulo'] = c3.number_input("Pre-ass Modulo", value=st.session_state.costs_config.get('min_preassemblaggio_modulo', 30.0))
        st.session_state.costs_config['min_preassemblaggio_mensola'] = c4.number_input("Pre-ass Mensola", value=st.session_state.costs_config.get('min_preassemblaggio_mensola', 5.0))
    ...
    # Ricalcolo stats veloci per preventivo (pescano da Tab 2 se calcolate, sennò approssimiamo)
    # Per sicurezza ricalcoliamo al volo i totali
    vol_ferro = sum([p['w'] * p['h'] * SPESSORE_FERRO for p in parts_list]); peso_ferro = (vol_ferro * PESO_SPECIFICO_FERRO) / 1000.0
    vol_legno = sum([w['w'] * w['d'] * SPESSORE_LEGNO for w in wood_list]); peso_legno = (vol_legno * PESO_SPECIFICO_LEGNO) / 1000.0
    num_viti = len(wood_list) * 6; 
    stats_calc = {"peso_ferro": peso_ferro, "peso_legno": peso_legno, "viti": num_viti}
    
    totals = calculate_quote(stats_calc, user_inputs)
    
    st.divider()
    # OUTPUT FINALE
    m1, m2 = st.columns(2)
    m1.metric("PREZZO TOTALE (IVATO)", f"€ {totals['price_total']:.2f}")
    m2.success(f"Consegna prevista: {totals['delivery_date']} ({totals['days_total']} gg lav.)")
    
    st.write(f"Imponibile: € {totals['price_ex_vat']:.2f} + IVA: € {totals['vat']:.2f}")
